...
close all
tic;  % 开始计时
MP = struct("Emax",[320, 330, 350, 370, 390, 420, 450, 470, 480, 490, 510],...
            "cost",[508.5, 545.2, 580.8, 660.4, 731.5, 823.6, 966.4, 1067.6, 1120.0, 1173.6, 1284.4],...
            "density",[11.3, 11.6, 12.1, 12.7, 13.3, 14.2, 15.1, 15.7, 16.0, 16.3, 16.9],...
            "Emin",1e-3,...
            "v",0.3,...
            "P",3);

GP = struct("ngp",4,...% number of Gaussian points
            "noph",11,...% number of material phases
            "els",1,...% size of unit element
            "lx",60,"ly",60,"cutout_size",36);% geometry of structure
OP = struct("NOI",6000,...% number of iterations
            "Tho",3.5,...% 增加过滤强度，使结构更平滑
            "delta_T",0.005,...% 减小时间步长，提高稳定性
            "VC0",[0.15,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.15],...
            "Mu",1*ones(1,11), ...  % 减小惩罚系数
            "nRelax",5*ones(1,11), ...  % 减少松弛迭代次数
            "Gamma",0.25*ones(1,11),...  % 减小初始Gamma值
            "dGamma",0.002*ones(1,11), ...  % 减小Gamma增量
            "maxGamma",4*ones(1,11), ...  % 减小最大Gamma值
            "Lambda",0*ones(1,11), ...
            "PlotResult",1);
...
% Calculation of vlfe
    ...
    for i=1:GP.noph
        ...
        if i>1
            ind=Phi(:,:,i-1)<0;
            Phi(ind,:,i)=Phi(ind,:,i-1)-0.9;  % 减小偏移量
        end
        ...
        Phi(boundary_nodes1,:,10)=0.008;
        Phi(boundary_nodes1,:,11)=0.005;
        ...
        Phi(boundary_nodes2,:,10)=-0.48;
        Phi(boundary_nodes2,:,9)=-0.49;
       ...
       % 定义材料颜色和名称
    materialColors = parula(11);
    
    materialNames = {'Mo45Ta35V20', 'W5Ta35Mo45V15', 'W15Ta30Mo45V10', ...
                 'W25Ta25Mo45V5', 'W35Ta20Mo45', 'W50Ta15Mo35', ...
                 'W65Ta15Mo20', 'W75Ta15Mo10', 'W80Ta15Mo5', ...
                 'W85Ta15', 'W95Ta5'};
