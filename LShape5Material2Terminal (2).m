...
tic;  % 开始计时
MP = struct( ...
    "Emax",   [320, 370, 460, 490, 510], ...           % GPa
    "cost",   [508.5, 660.4, 1016.4, 1173.6, 1284.4], ... % e-3 $/cm3
    "density",[11.3, 12.7, 15.4, 16.3, 16.9], ...       % g/cm^3
    "Emin",   1e-3, ...
    "v",      0.3, ...
    "P",      1.5 ...
);
GP = struct("ngp",4,...% number of Gaussian points
            "noph",5,...% number of material phases
            "els",0.5,...% size of unit element
            "lx",60,"ly",60,"cutout_size",36);% geometry of structure
OP = struct("NOI",450,...% number of iterations
            "Tho",0.8,...% control the complexity and smoothness of the structure
            "delta_T",0.1,...%% time step
            "VC0",[0.15,0.01,0.01,0.01,0.15],...
            "Mu",2*[10,10,10,10,10], ...
            "nRelax",40*[1,1,1,1,1], ...
            "Gamma",[1,1,1,1,1],...
            "dGamma",[0.05,0.05,0.05,0.05,0.05], ...
            "maxGamma",[15,15,15,15,15], ...
            "Lambda",[0,0,0,0,0], ...
            "PlotResult",1);
...
% Calculation of vlfe
    ...
    for i=1:GP.noph
        ...
        Phi(boundary_nodes1,:,5)=0.05;Phi(boundary_nodes1,:,4)=0.1;Phi(boundary_nodes1,:,3)=0.15;Phi(boundary_nodes1,:,2)=0.2;Phi(boundary_nodes1,:,1)=0.7;
        Phi(boundary_nodes2,:,1)=0.05;Phi(boundary_nodes2,:,2)=-0.45;Phi(boundary_nodes2,:,3)=-0.5;Phi(boundary_nodes2,:,4)=-0.55;Phi(boundary_nodes2,:,5)=-0.6;
    ...
    materialNames = {'Mo45Ta35V20', 'W25Ta25Mo45V5', 'W70Ta15Mo15', 'W85Ta15', 'W95Ta5'};
    figure(97); clf
    hold on
    axis equal
    for i = 1:GP.noph
