...
tic;  % 开始计时
MP = struct( ...
    "Emax",    [315, 322, 330, 370, 410, 450, 510], ... % GPa
    "cost",    [450.0, 465.6, 545.2, 660.4, 792.3, 966.4, 1284.4], ... % $/1e3 cm^3
    "density", [10.2, 10.4, 11.6, 12.7, 13.9, 15.1, 16.9], ...
    "Emin",    1e-3, ...
    "nu",      0.3, ...
    "v",0.3,...
    "P",       1.5 ...
...
GP = struct("ngp",4,...% number of Gaussian points
            "noph",7,...% number of material phases
            "els",1,...% size of unit element
            "lx",60,"ly",60,"cutout_size",36);% geometry of structure
OP = struct("NOI",1000,...% number of iterations
            "Tho",1.5,...% 增加过滤强度，使结构更平滑
            "delta_T",0.02,...% 减小时间步长，提高稳定性
            "VC0",[0.15,0.01,0.01,0.01,0.01,0.01,0.15],...
            "Mu",[3,3,3,3,3,3,3], ...  % 减小惩罚系数
            "nRelax",15*[1,1,1,1,1,1,1], ...  % 减少松弛迭代次数
            "Gamma",[0.3,0.3,0.3,0.3,0.3,0.3,0.3],...  % 减小初始Gamma值
            "dGamma",[0.01,0.01,0.01,0.01,0.01,0.01,0.01], ...  % 减小Gamma增量
            "maxGamma",[8,8,8,8,8,8,8], ...  % 减小最大Gamma值
...
bottom_edge_nodes=(NodeCoord(:, 1) <=  35) & (NodeCoord(:, 1) >= 25) &(NodeCoord(:, 2) >= 0) & (NodeCoord(:, 2) <= GP.els);
top_edge_nodes = NodeCoord(:, 1) >= 0 & NodeCoord(:, 1) <= 24 & NodeCoord(:, 2) >= (GP.ly-GP.els) & (NodeCoord(:, 2) <= GP.ly);
while opc<=OP.NOI
    for i=1:GP.noph
...
% Calculation of vlfe
    ...
    for i=1:GP.noph
        ...
        if i==2
            ind=Phi(:,:,i-1)<0;
            Phi(ind,:,i)=Phi(ind,:,i-1)-0.6;  % 减小偏移量
        end
        if i==3
            ind=Phi(:,:,i-1)<0;
            Phi(ind,:,i)=Phi(ind,:,i-1)-0.6;
        end
        if i==4
            ind=Phi(:,:,i-1)<0;
            Phi(ind,:,i)=Phi(ind,:,i-1)-0.6;
        ...
        Phi(right_edge_nodes,:,1)=0.55;  % material 7 in right edge nodes
        Phi(right_edge_nodes,:,2)=0.45;
        Phi(right_edge_nodes,:,3)=0.35;
        Phi(right_edge_nodes,:,4)=0.25;
        Phi(right_edge_nodes,:,5)=0.15;
        Phi(right_edge_nodes,:,6)=0.05;
        Phi(right_edge_nodes,:,7)=0.03;

        Phi(bottom_edge_nodes,:,1)=0.55;  % 减小初始值
        Phi(bottom_edge_nodes,:,2)=0.45;
        Phi(bottom_edge_nodes,:,3)=0.15;
        Phi(bottom_edge_nodes,:,4)=-0.05;
        Phi(bottom_edge_nodes,:,5)=-0.15;
        Phi(bottom_edge_nodes,:,6)=-0.25;
        Phi(bottom_edge_nodes,:,7)=-0.35;
        
        Phi(top_edge_nodes,:,1)=0.08;  % material 1 in top edge nodes
        Phi(top_edge_nodes,:,2)=-0.08;
        Phi(top_edge_nodes,:,3)=-0.15;
        Phi(top_edge_nodes,:,4)=-0.22;
        Phi(top_edge_nodes,:,5)=-0.28;
        Phi(top_edge_nodes,:,6)=-0.35;
        Phi(top_edge_nodes,:,7)=-0.42;
       ...
       % 定义材料颜色和名称
    materialNames = {'Mo45Ta35V20', 'Ta10Mo65V25', 'Ta35Mo45V20', 'Ta25Mo45V5', ...
                 'Ta15Mo40', 'Ta15Mo20', 'W95Ta5'};
    
     materialColors = [
    0.000, 0.447, 0.698;   % blue (start, original)

    0.835, 0.369, 0.000;   % orange
    0.494, 0.184, 0.556;   % purple
    0.000, 0.620, 0.451;   % teal green
    0.301, 0.745, 0.933;   % light blue
    0.635, 0.078, 0.184;   % dark red

    0.000, 0.000, 0.000    % black (end, original)
];
