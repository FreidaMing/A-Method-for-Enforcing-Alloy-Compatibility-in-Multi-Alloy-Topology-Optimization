...
MP = struct( ...
    "Emax",    [320, 420, 510], ...
    "cost",    [508.5, 823.6, 1284.4], ...
    "density", [11.3, 14.2, 16.9], ...
    "Emin",    1e-3, ...
    "v",       0.3, ...
    "P",       1.5 ...
);
GP = struct("ngp",4,...% number of Gaussian points
            "noph",3,...% number of material phases
            "els",1,...% size of unit element
            "lx",60,"ly",60,"cutout_size",36);% geometry of structure
OP = struct("NOI",300,...% number of iterations
...
% Calculation of vlfe
    ...
    for i=1:GP.noph
        ...
        if i==2
            ind=Phi(:,:,i-1)<0;
            Phi(ind,:,i)=Phi(ind,:,i-1)-0.2;
        end
        if i==3
            ind=Phi(:,:,i-1)<0;
            Phi(ind,:,i)=Phi(ind,:,i-1)-0.2;
        end
        Phi(boundary_nodes1,:,1)=0.5;Phi(boundary_nodes1,:,2)=0.3;Phi(boundary_nodes1,:,3)=0.1;
        Phi(boundary_nodes2,:,1)=0.1;Phi(boundary_nodes2,:,2)=-0.1;Phi(boundary_nodes2,:,3)=-0.2;
    ...
    materialNames = {'Mo45Ta35V20', 'W50Ta15Mo35', 'W95Ta5'};
